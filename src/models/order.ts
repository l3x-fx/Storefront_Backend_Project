...
export class OrderStore {
  async createOrder(user_id: number): Promise<number> {
    try {
      const sql = "INSERT INTO orders (status, date, user_id) VALUES($1, $2, $3) RETURNING *"
      // @ts-ignore
  ...
  async flagOrderInvalid(order_id: number): Promise<number> {
    try {
      ...
      //@ts-ignore
      const conn = await Client.connect()
      const result = await conn.query(sql, ["invalid", order_id])
      conn.release()
  ...
  async addProductToOrder(order_id: number, product: CartItem): Promise<OrderProducts> {
    try {
      ...
      //@ts-ignore
      const conn = await Client.connect()
      const result = await conn.query(sql, [product.quantity, order_id, product.id])
      conn.release()
  ...
  async getAllOrdersByUserId(user_id: number): Promise<Order[]> {
    try {
      ...
      const orders = await Promise.all(
        orderShort.rows.map(async (order: { id: number; status: string; date: string; user_id: string }) => {
          try {
